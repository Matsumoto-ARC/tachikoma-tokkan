...
void gfunc_motor_operation(ANALOG_STICK tcl_ana_st, ANALOG_STICK_VAL tcl_motor_val) {
    ...
    /* 停止 */
    if ((tcl_ana_st.l3_up == false) && (tcl_ana_st.l3_down == false) && (tcl_ana_st.l3_left == false) && (tcl_ana_st.l3_right == false)) { 
    ...
    /* 前進 */
    }else if (((tcl_ana_st.l3_up != false) && (tcl_ana_st.l3_left == false) && (tcl_ana_st.l3_right == false)) || 
    ...
    /* 左旋回 */
        ...
        /* 以下設定はモータードライバーとの配線次第で変更が必要 */
        digitalWrite(MOTOR_PIN3, HIGH);       /* 左モーター:IN1をLOWに設定 */
        digitalWrite(MOTOR_PIN4, LOW);      /* 左モーター:IN2をHIGHに設定 */
        digitalWrite(MOTOR_PIN5, LOW);       /* 右モーター:IN1をLOWに設定 */
        digitalWrite(MOTOR_PIN6, HIGH);      /* 右モーター:IN2をHIGHに設定 */
    ...
    /* 右旋回 */
        ...
        /* 以下設定はモータードライバーとの配線次第で変更が必要 */
        digitalWrite(MOTOR_PIN3, LOW);       /* 左モーター:IN1をHIGHに設定 */
        digitalWrite(MOTOR_PIN4, HIGH);        /* 左モーター:IN2をLOWに設定 */
        digitalWrite(MOTOR_PIN5, HIGH);       /* 右モーター:IN1をHIGHに設定 */
        digitalWrite(MOTOR_PIN6, LOW);        /* 右モーター:IN2をLOWに設定 */
    }
